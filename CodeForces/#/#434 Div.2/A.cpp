...
int main()
{
    ...
    while(cin >> n >> k)
    {
        ...
        int base = min(two,five);
        two -= base, five -= base;
        if(base>=k) cout << ori << endl;
        else{
            k-=base;
            int num = 0, mul;
            if(two) num = min(k,two), mul = 5;
            if(five)num = min(k,five),mul = 2;
            for(int i=0;i<num;i++) ori*=mul;
            cout << ori;
            for(int i=num;i<k;i++) cout << 0;
            cout << endl;
        }
    }
    // system("pause");
